...
def gen_bb_bands_for_sym(sym, price_df, lookback, std):
    '''
    Function to calculate bollinger band values for each symbol

    args:
    sym(str): Name of symbol
    price_df(pandas dataframe): Close/Adjusted Close values for the
                                symbol
    lookback(int): No. of days over which moving average is to be
                   calculated
    std(int): No. of standard deviations

    except:
    None

    returns:
    df[sym](pandas series): Pandas series with bollinger band values
                            for the symbol
    '''
    
    df = pd.DataFrame()
    df['close'] = price_df
    df['close_avg']  = df['close'].rolling(window=lookback).mean()
    df['std']        = df['close'].rolling(window=lookback).std() * std
    df['upper_band'] = df['close_avg'] + df['std']
    df['lower_band'] = df['close_avg'] - df['std']

    df[sym] = (df['close'] - df['close_avg'])/df['std']
    #df[sym]  = np.where(df['close'] >= df['upper_band'],1, 
    #                               (np.where(df['close'] <= df['lower_band'],-1, 
    #                                         (df['close'] - df['close_avg'])/df['std'])))
    
    return df[sym]

def gen_bb_bands(ls_symbols, closes, lookback, std):
    '''
    Function to generate bollinger band values for a set of
    symbols

    args:
    ls_symbols(list): List of symbols
    closes(pandas dataframe): Set of close values(Actual Close/Adjusted
                              Close) for a set of symbols
    lookback(int): No. of days to look back to calculate moving average
    std(int): No. of standard deviations
    ...
    bb_bands = pd.DataFrame()
    for sym in ls_symbols:
        bb_sym = gen_bb_bands_for_sym(sym, closes[sym], lookback, std)
        bb_bands = pd.concat([bb_bands, bb_sym], axis=1)
